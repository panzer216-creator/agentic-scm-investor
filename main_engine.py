...
class AgenticSCMEngine:
    ...
    def run_production_line(self, group_id, trigger_reason, current_report_id):
        ...
        try:
            ...
            parser = ParserAgent()
            sdp_payload = parser.parse(raw_data, self.sector).get("standard_data_pack", {})
            ...
            full_text = "\n".join(reasoning)
            if len(full_text) > 3500: reasoning = [full_text[:3500] + "\n...[데이터 과다로 중략]"]

            # [수정 완료] 타점 가이드 포장 로직 추가
            output = {
                ...
                'entry_point_guide': final_decision.get('entry_point_guide', '기술적 지표 분석 중...'),
                'reasoning': reasoning,
                'ui_metrics': self._extract_ui_metrics(p_info),
...
def should_run_analysis(stock_code, current_price_data, history_list):
    ...
    if last_price > 0 and curr_price > 0:
        if abs((curr_price - last_price) / last_price) * 100 >= 5.0: return True, "주가 급변 감지", current_report_id
    return False, "변동 사항 없음", current_report_id

if __name__ == "__main__":
    logging.info("🏭 Agentic SCM Investment Engine v2.2 가동")
    if not os.path.exists("data/universe.json"): BucketFetcher() 
    
    production_plan = BucketFetcher().get_dynamic_production_plan()
    if not production_plan: exit(1)
    ...
    for group_id, stocks in production_plan.items():
        for stock in stocks:
            try:
                ...
                if run_flag:
                    AgenticSCMEngine(stock["code"], stock["name"], stock["sector"]).run_production_line(group_id, reason, report_id)
                    time.sleep(1)
                else:
